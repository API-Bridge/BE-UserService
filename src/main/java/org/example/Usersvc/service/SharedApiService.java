...
public class SharedApiService {
    ...
    @Transactional
    public SharedApi shareApi(String userId, String apiId, String planType) {
        log.debug("API 공유 시작 - userId: {}, apiId: {}, planType: {}", userId, apiId, planType);

        CustomApi customApi = customApiRepository.findByApiIdAndDeletedFalse(apiId)
        ...
        Optional<SharedApi> existingSharedApi = sharedApiRepository.findByOriginalApiIdAndActiveTrue(apiId);
        ...
        SharedApi sharedApi = SharedApi.builder()
                ...
                .originalApiId(apiId)
                .creatorId(userId)
                .apiName(customApi.getApiName())
                .description(customApi.getDescription())
                .dataCount(customApi.getDataCount())
                .build();

        SharedApi savedSharedApi = sharedApiRepository.save(sharedApi);
        log.info("API 공유 완료 - sharedApiId: {}, originalApiId: {}", savedSharedApi.getSharedApiId(), apiId);

        return savedSharedApi;
    }

    @Transactional
    public void unshareApi(String userId, String apiId) {
        log.debug("API 공유 취소 시작 - userId: {}, apiId: {}", userId, apiId);

        SharedApi sharedApi = sharedApiRepository.findByOriginalApiIdAndActiveTrue(apiId)
        ...
        log.info("API 공유 취소 완료 - sharedApiId: {}, originalApiId: {}", sharedApi.getSharedApiId(), apiId);
    }

    public List<SharedApi> getActiveSharedApis() {
        return sharedApiRepository.findByActiveTrueOrderByCreatedAtDesc();
    }

    public Page<SharedApi> getActiveSharedApis(Pageable pageable) {
        return sharedApiRepository.findByActiveTrueOrderByCreatedAtDesc(pageable);
    }

    public List<SharedApi> getSharedApisByCreator(String creatorId) {
        return sharedApiRepository.findByCreatorIdAndActiveTrueOrderByCreatedAtDesc(creatorId);
    }

    public List<SharedApi> searchSharedApis(String keyword) {
        return sharedApiRepository.findByApiNameContainingIgnoreCaseAndActiveTrueOrderByCreatedAtDesc(keyword);
    }

    public List<SharedApi> getSharedApisByDataCountLimit(int maxDataCount) {
        return sharedApiRepository.findByDataCountLessThanEqualAndActiveTrueOrderByCreatedAtDesc(maxDataCount);
    }
    ...
    public Optional<SharedApi> getSharedApiByOriginalApiId(String originalApiId) {
        return sharedApiRepository.findByOriginalApiIdAndActiveTrue(originalApiId);
    }
}