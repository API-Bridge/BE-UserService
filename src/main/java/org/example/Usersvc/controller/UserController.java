...
public class UserController {
    ...
    public ResponseEntity<BaseResponse<User>> createUser(@Valid @RequestBody CreateUserRequest request) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(BaseResponse.error("INTERNAL_ERROR", "사용자 생성에 실패했습니다."));
        }
    }
    ...
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<BaseResponse<User>> getUser(@PathVariable String userId) {
        ...
        try {
            ...
            if (!isValidUUID(userId)) {
                ...
                return ResponseEntity.badRequest()
                        .body(BaseResponse.error("INVALID_UUID", "올바른 UUID 형식이 아닙니다."));
            }
            ...
            if (user.isEmpty()) {
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(BaseResponse.error("USER_NOT_FOUND", "사용자를 찾을 수 없습니다."));
            }
            
            log.debug("사용자 조회 성공 - userId: {}", userId);
            return ResponseEntity.ok(BaseResponse.success(user.get()));
            
        } catch (Exception e) {
            log.error("사용자 조회 중 오류 발생 - userId: {}", userId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(BaseResponse.error("INTERNAL_ERROR", "사용자 조회에 실패했습니다."));
        }
    }
    ...
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<BaseResponse<UserSecretsArn>> registerUserSecret(
            @PathVariable String userId,
            @Valid @RequestBody RegisterSecretRequest request) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(BaseResponse.success(registeredArn));
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(BaseResponse.error("INTERNAL_ERROR", "개인 키 등록에 실패했습니다."));
        }
    }
    ...
    @PreAuthorize("hasRole('SERVICE') or hasRole('USER')")  // AI 서비스 또는 사용자 접근 허용
    public ResponseEntity<BaseResponse<Map<String, Object>>> getUserSecretValue(
            @PathVariable String userId,
            @RequestParam @NotBlank(message = "ARN ID는 필수입니다") String arnId) {
        ...
        try {
            ...
            if (arnInfo.isEmpty()) {
                ...
                return ResponseEntity.status(HttpStatus.NOT_FOUND)
                        .body(BaseResponse.error("ARN_NOT_FOUND", "요청한 ARN을 찾을 수 없습니다."));
            }
            ...
            if (!arnInfo.get().getUserId().equals(userId)) {
                ...
                return ResponseEntity.status(HttpStatus.FORBIDDEN)
                        .body(BaseResponse.error("ACCESS_DENIED", "해당 ARN에 접근 권한이 없습니다."));
            }
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(BaseResponse.error("INTERNAL_ERROR", "개인 키 조회에 실패했습니다."));
        }
    }
    ...
    @PreAuthorize("hasRole('USER')")
    public ResponseEntity<BaseResponse<List<UserSecretsArn>>> getUserSecrets(@PathVariable String userId) {
        ...
        try {
            ...
            return ResponseEntity.ok(BaseResponse.success(userSecrets));
            
        } catch (Exception e) {
            log.error("사용자 키 목록 조회 중 오류 발생 - userId: {}", userId, e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(BaseResponse.error("INTERNAL_ERROR", "키 목록 조회에 실패했습니다."));
        }
    }
